...
SCENARIO ("Player accurately represents whether it is running", "[player]") {
	GIVEN ("a fresh Player using dummy audio sources and sinks") {
		...
		WHEN ("the player has been asked to quit") {
			...
			THEN ("Update returns false (the player is no longer "
			      "running)") {
				REQUIRE_FALSE(p.
	...
	GIVEN ("a fresh Player using dummy audio sources and sinks") {
		...
		WHEN ("there is no audio loaded") {
			THEN ("playing returns failure") {
				REQUIRE_FALSE(p.SetPlaying("tag", true)
				                      .
			...
			THEN ("stopping returns failure") {
				REQUIRE_FALSE(p.SetPlaying("tag", false)
				                      .
			...
			THEN ("loading for an unknown file type returns "
			      "failure") {
				REQUIRE_FALSE(p.Load("tag", "blah.wav")
				                      .
		...
		WHEN ("there is audio loaded") {
			...
			{
				THEN ("setting state to Playing returns "
				      "success") {
					REQUIRE(p.SetPlaying("tag", true)
					                .
				...
				THEN ("setting state to Stopped returns "
				      "success") {
					// Telling a stopped file to stop is a
					// no-op.
					REQUIRE(p.SetPlaying("tag", false)
					                .
				...
				THEN ("setting state to Ejected returns "
				      "success") {
					REQUIRE(p.Eject("tag").
				...
				THEN ("loading for a known file type returns "
				      "success") {
					REQUIRE(p.Load("tag", "blah.mp3")
					                .
				...
				THEN ("loading for an unknown file type "
				      "returns "
				      "failure") {
					REQUIRE_FALSE(p.Load("tag", "blah.wav")
					                      .
	...
	GIVEN ("a Player") {
		...
		WHEN ("no file is loaded") {
			...
			AND_WHEN("a load fails with FileError")
			{
				...
				THEN ("the load response is FAIL with the "
				      "given "
				      "message") {
					REQUIRE(rs.
