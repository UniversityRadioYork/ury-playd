...
SCENARIO("PlayerState begins in a valid state", "[player-state]")
{
	GIVEN("a fresh PlayerState")
	{
		...
		{
			bool is_ejected =
			                ps.In({ PlayerState::State::EJECTED });
...
SCENARIO("PlayerState pushes state changes to its registered response sink",
         "[player-state], [response-source]")
{
...
SCENARIO("PlayerState correctly returns whether or not it is running",
         "[player-state]")
{
	GIVEN("a fresh PlayerState")
	{
